...
int main(int argc, char** argv)
{
    ...
    // Creates a normal (Gaussian) distribution for step
    //68.268949% 95.449974% 99.730020% 99.993666% 1sigma 2sigma 3sigma 4sigma
    normal_distribution<double> dist_rot(0, translationSigma);
    normal_distribution<double> dist_transl(0, rotationSigma); 
    normal_distribution<double> dist_point(0, pointSigma); 
    ...
    for (int i = 0; i < nFrames; ++i)
    {
        ...
        // camera to world
        Eigen::Quaterniond q;
        q = Eigen::AngleAxisd(dist_rot(gen), Eigen::Vector3d::UnitZ())
            * Eigen::AngleAxisd(orientation + dist_rot(gen), Eigen::Vector3d::UnitY())
            * Eigen::AngleAxisd(dist_rot(gen), Eigen::Vector3d::UnitX());
        Eigen::Vector3d t;
        t << x, y, z;
        ...
        for (int j = 0; j < nScenePoints; ++j)
        {
            ...
            bool hasCorrespondence = true; //random
